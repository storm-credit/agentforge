...
- Full pytest suite green (read-only observation): move `apps/api/.env` aside before running, restore it after (two tests hit a real LLM/DB and fail otherwise). Report the exact baseline count you observed, not an assumed one.
- `ruff check` clean on touched files (observed, not fixed by this role).
- For an authz/ACL/trust-boundary change: a live reproduction of the before/after behavior (the actual request that used to leak or bypass, now denied/fail-closed), not just a unit test claim. Prefer running an adversarial pass (`security-review` skill) over the diff before signing off.
