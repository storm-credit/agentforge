        ...
        {isPrivileged && (
          <>
            ...
            <ul className="statusList" data-testid="audit-list" style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {events.map((e) => (
                <li key={e.id} data-testid="audit-row" style={{ padding: "var(--space-2) 0" }}>
                  <div style={{ display: "flex", gap: "var(--space-2)", alignItems: "center", flexWrap: "wrap" }}>
                    <span className="badge info" data-testid="audit-event-type">{e.event_type}</span>
                    <span className="meta">
                      {e.created_at.slice(0, 19).replace("T", " ")} · {e.actor_id} ({e.actor_department}) · {e.target_type}:{e.target_id.slice(0, 8)}
                    </span>
                  </div>
                  {e.reason && <p style={{ fontSize: "var(--text-xs)", margin: "2px 0 0" }}>사유: {e.reason}</p>}
                </li>
              ))}
            </ul>
          </>
