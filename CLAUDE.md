...
> **⛔ 상위 권위 & 피처 프리즈 (2026-07-22 이후, 이 파일보다 우선).** 현재 권위 SSOT는 [docs/40-delivery/current-state.md]이고, **CLAUDE.md는 그에 하위(subordinate)**다. SSOT가 **피처 프리즈**를 활성화했으므로 — 아래 rule 1~2의 "슬라이스마다 진행 + 슬라이스 종료마다 전문가 패널로 백로그 도출" **루프는 프리즈 동안 중단**한다. 프리즈 하에서 새 작업은 **오직** current-state §7의 *accepted Work Order*(직접 파일럿 블로커, 실 SSO/문서/모델/staging 연동, 파일럿 필수 운영통제, 또는 재현되는 보안/데이터노출/무결성/배포/eval·트레이스·인용·거부·감사 정합성 결함)만 허용된다. **금지(§8)**: "보안·무결성·파일럿·증거·운영·사용자정합성 임팩트 없는 반복 전문가-패널/nit 작업." 즉 확인 패널 반복·미세 폴리시 루프를 돌리지 말 것. 아래 규칙은 프리즈가 해제(파일럿 결정 착수)된 뒤 또는 위 예외 범위 내에서만 적용된다.
...
5. **설계·브레인스토밍·리뷰 기본기 (사용자 확정 2026-07-25).** 설계·기획·리뷰·아키텍처 작업 시 아래를 기본 적용. **단, 비례성 원칙**: 사소한 작업(한 줄 수정·단발 조회)엔 과하게 굴지 말고 합리적 기본값으로 진행하며 알린다. auto-mode/모멘텀 선호와 균형.
   - **맹점 훑기**: 요청·설계에서 빠진 것·놓친 것·암묵 가정을 능동적으로 먼저 드러낸다(내가 틀렸을 수 있는 지점 포함).
   - **함정 먼저 짚기**: 착수 전에 이 접근의 리스크·실패지점·되돌리기 어려운 결정을 앞세워 경고한다.
   - **디자인 시안 4개(한눈에)**: UI/구조 설계는 선택지 ~4개를 나란히 비교 가능하게 제시(장단점·미니 목업/스케치), 그중 추천 1개를 근거와 함께. 필요시 `AskUserQuestion`의 preview로 한눈에. (사소 결정엔 생략)
   - **나를 인터뷰**: 결과를 크게 바꾸는 미정 사항은 추측하지 말고 사용자에게 질문(한 번에 핵심만, 선택지 제시). 사소한 건 기본값으로 진행하고 알림.
   - **본보기 코드**: 비슷한 오픈소스를 GitHub(WebSearch/WebFetch)에서 찾아 선행사례·패턴을 참고. **폐쇄망 전제** — 라이선스·자립성(외부 egress 없는지) 확인.
   - **이탈 로그**: 막히거나 원래 계획과 달라지면 **어디서·왜** 그랬는지 정리해 [docs/status-and-go-no-go.md] 또는 핸드오프/이탈로그 문서에 남긴다. 추측을 사실로 쓰지 않기([[completion-claims-discipline]] 정직 원칙 연장).
6. **메타프롬프팅 (사용자 확정 2026-07-25).** AI에게 줄 프롬프트를 AI가 직접 쓰게 한다: **맥락 덤핑 → 프롬프트 깎아내기 → 산출물 점검** 3단계. **비례성**: 한 줄이면 끝나는 사소 요청엔 이 루프를 강제하지 말고 기본값으로 바로 진행.
   - **질문 유도**: "지금까지의 대화로 프롬프트를 만들 텐데, 좋은 프롬프트를 위해 더 필요한 맥락이 있으면 나에게 물어봐줘"라고 AI에게 명시적으로 요청해 빠진 맥락을 먼저 뽑아낸다.
   - **성공조건 명시**: 검증 가능한 완료 조건을 프롬프트에 박아넣는다(예: "모바일 첫 화면 안에 핵심 가치가 보여야 함", "CTA 명확", "모든 버튼·폼이 실제로 동작").
   - **실행 환경별 변환**: 최종 프롬프트를 대상 환경에 맞게 번역한다 — 목표형(goal)은 **중지 요건**, ultracode류는 **제약 조건**, 이미지 생성은 **구도·피사체·스타일·조명·카메라 방향**, 리서치 에이전트는 **출처 기준·조사 범위·검증 방식**.
