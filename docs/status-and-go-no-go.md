...
1. **🚨 (최우선, opus) `force_reindex` 재색인 인가 상향** — 이미 색인된 문서의 재색인은 `PRIVILEGED_ROLES`로, 최초 색인은 기존 read-ACL 유지.
2. `create_source`/`register_document`/`upload_document_and_index`에 RBAC 부재 — "누가 생성 가능한가" 정책 갭(포이즈닝과는 다른 범주지만 관련), 위 1번과 같은 파일이라 함께 처리 검토. S, 백엔드.
3. CI에 `alembic upgrade head`(+downgrade/재upgrade)를 실 Postgres 서비스 컨테이너 대상으로 추가 — PR #75의 마이그레이션이 스크래치 SQLite로만 검증됐던 갭을 직접 해소, 새로 부상. S, DevOps.
4. `/audit` 페이지를 데모 역할 스위처 패턴(PR #63)에 포함 — 비admin이 raw 403 대신 친절한 "admin 전용" 표시. S, 프론트.
