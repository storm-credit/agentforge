...
// Runs are created as the SELECTED DEMO ROLE, like every other call in this file.
// Previously this one function sent a separate per-page persona bundle, so the run
// (and its audit trail) was attributed to a different principal than the rest of
// the session — and GET /runs is owner-or-admin scoped server-side, so the run
// could then be invisible to the identity that had just created it.
