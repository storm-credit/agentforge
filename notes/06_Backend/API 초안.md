...
```json
{
  ...
  "groups": ["all-employees", "finance-readers"],
...
| `POST` | `/knowledge/retrieval/preview` | 현재 principal 기준 검색 후보와 citation preview. MVP 구현 경로 | `agent_user` |
...
Sprint 1의 첫 구현 경로는 실제 vector search 전에 문서 메타데이터 기반 ACL 계약을 고정한다.
...
```json
{
  ...
  "hits": [
    {
      "document_id": "doc_01HX...",
      "knowledge_source_id": "ks_01HX...",
      "title": "Company Holiday Policy",
      "confidentiality_level": "internal",
      "access_groups": ["all-employees"],
      "score": 0.5,
      "citation": "Company Holiday Policy (undated)"
    }
  ],
  "denied_count": 2
}
...
- `confidentiality_level`이 principal clearance보다 높으면 제외한다.
- `access_groups`가 비어 있으면 deny-by-default로 제외한다.
- `all-employees`, `user:{id}`, `department:{department}`, `role:{role}`, principal groups 중 하나와 교집합이 있어야 한다.
- 응답에는 허용된 문서만 포함하고, audit payload에는 query 원문이 아니라 길이와 count만 남긴다.
