...
- Agent Studio now includes a Test Chat draft on `/agents` for published/validated seed or API-synced agents, with `POST /api/v1/runs` integration, `agent_version_id`/`knowledge_source_ids` forwarding, local fallback, citation chips, guardrail/citation status, no-context refusal state, and Trace links.
