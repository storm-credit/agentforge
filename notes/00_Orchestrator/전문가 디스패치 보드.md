...
추가 총괄 지시: 각 전문 에이전트와 런타임 내부 에이전트는 모델 tier를 명시한다. routine 작업은 작은 모델로 처리하고, 보안/릴리스/아키텍처/실패 triage는 deep-review로 승격한다. 공식 기준은 `docs/agent-model-routing-policy.md`와 `packages/shared-contracts/model-routing-policy.v0.1.json`이다.
...
| Fake vector adapter contract | Backend + RAG + Security + QA | 시작 | ACL filter 필수 검색과 deterministic upsert 테스트 통과 |
...
| Real document upload ingestion | Backend + RAG + Security | 진행중 | MinIO/object storage 저장, checksum, audit, index job 연결 |
| Knowledge upload workflow | Frontend + QA | 진행중 | source/document/upload/index/retrieval preview 운영 흐름 |
| Upload-to-runtime D3 smoke | QA + DevOps/MLOps | 진행중 | 실제 업로드 기반 indexing smoke와 contract test 보강 |
| Agent model routing policy | Orchestrator + AI Runtime + Security + QA | 시작 | 전문/런타임 에이전트별 tier, escalation, D3 evidence 기준 |
