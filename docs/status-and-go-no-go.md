...
1. **🚨 index-job 생성/처리 엔드포인트 ACL 추가** (보안 취약점 수정, opus) — 위 발견. S, 최우선.
2. 문서 `GET /knowledge/documents`에 `include_archived`(admin 전용) 필터 추가 — 복원 API(PR #55)가 존재해도 admin이 보관된 문서 UUID를 알 방법이 없어 사실상 죽은 기능. S, 백엔드.
3. 프론트 보관문서 뷰 + 복원 버튼 (`restoreDocument()` 추가, Knowledge 페이지에 "보관됨 보기" 토글) — #2 선행 필요. S-M, 프론트.
4. 비-LLM 하이브리드(BM25/어휘) 리랭커 — `reranker.py`엔 NoopReranker뿐, 프로토콜/호출부(`runs.py`)는 이미 배선되어 있어 드롭인 가능. c07 스칼라 튜닝 재탕이 아닌 구조적으로 새로운 신호. M, RAG.
5. 시작 시 설정 검증(fail-fast) — `vector_backend`/`object_store_backend`/`rerank_backend`/`judge_backend`가 자유문자열이라 오타 시 조용히 fake/none 경로로 빠지고 readyz도 "skipped"(초록)로 오판. `Literal[...]` + `model_validator`로 부팅 시 즉시 실패하게. S, DevOps.
6. eval 실행결과 영속화 API — `/eval` 프론트는 완전 정적 스텁("Planned" 하드코딩), 저장할 백엔드 자체가 없음(신규 발견). `POST/GET /eval/runs` 최소 테이블+이력 뷰. S-M, QA.
7. `faithfulness_pct` 리포트에 사용된 임계값을 JSON/마크다운 출력 자체에 노출 — 기본값 0.0이면 사실상 항상 100%인데 리포트만 보는 사람은 오독 가능(코드/README엔 이미 명시했지만 리포트 자체엔 없음). S, QA.
8. (여력 시) 목록 엔드포인트 페이지네이션 전무 — `list_documents`/`list_sources`/`GET /agents` 전부 무제한 반환. M, 백엔드, 범위 큼.
