...
- 개발: 로컬 Qwen3-8B via **Ollama** (Windows + 단일 GPU엔 vLLM보다 쉬움, OpenAI 호환 :11434).
  - 확인된 로컬 환경: **RTX 4070 Ti / 12GB VRAM** → Qwen3-8B는 **양자화(Q4/Q5) 필수**(fp16은 12GB 초과). 8B-Q4는 ~5–6GB로 여유.
  - ⚠️ 현재 Ollama/vLLM **미설치** → 환경 셋업이 구현 첫 작업: Ollama 설치 → `ollama pull qwen3:8b` → `LLM_BASE_URL=http://localhost:11434/v1`.
- 이관: 사내 Qwen3.6:35B (vLLM). **env 교체만, 무코드** (게이트웨이가 OpenAI 호환이라 Ollama↔vLLM 동일).
...
- 로컬 LLM 런타임(Ollama) 미설치 → 구현 첫 작업으로 설치+모델 pull 필요. 12GB라 양자화 필수(8B-Q4).
