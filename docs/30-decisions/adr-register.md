...
| ADR-106 | Reranker model and whether pilot baseline uses reranking — detailed draft: `ADR-106-reranker-baseline.md` (`PROPOSED`, not GO) | `OPEN` | RAG/Data / QA-Eval | no reranker, cross-encoder, approved alternative | real-corpus quality/latency comparison and operational ownership | Retrieval baseline / Config-C |
| ADR-107 | Vector backend for closed-network staging — detailed draft: `ADR-107-vector-backend.md` (`ADOPTED` for backend/ACL-contract only; backup/restore/capacity/offline-packaging still open) | `OPEN` | Platform / RAG/Data | Qdrant, pgvector | ACL query contract, backup/restore, capacity, operations skill, offline packaging | Staging deployment |
| ADR-108 | Object storage implementation — detailed draft: `ADR-108-object-storage.md` (`PROVISIONAL`; on/off + delete-capability gap still open) | `OPEN` | Platform | MinIO-compatible, approved internal object store, filesystem with controls | access, checksum, backup/restore, retention, HA | Staging ingestion |
...
| ADR-114 | Config-C retrieval/rerank/evaluation baseline — detailed draft: `ADR-114-config-c-baseline.md` (`PROPOSED`, conditional candidate only) | `OPEN` | Product / RAG / QA-Eval | Existing candidate configurations | fixed corpus, metrics, latency, regression report, decision rationale | Final pilot baseline |
