...
## 알려진 한계 (배포 전)
SSO 미연동(헤더 스텁) · 프롬프트 인젝션은 약한 모델서 비결정적 우회(하드닝=베이스라인) · 업로드 TXT/MD/PDF/DOCX 지원(XLSX·원본파일 보관 미지원) · 객체저장소(MinIO) 미배선 — 업로드는 동기 인메모리 처리(AF-009 예정, compose의 minio 서비스는 스캐폴딩) · `/runs` 등 GET 무인증.
