...
MVP 검증은 모델을 두 갈래로 분리한다.

| Lane | 모델 | 목적 | 통과 기준 |
|---|---|---|---|
| `local-regression` | 로컬 Qwen3 8B 또는 동급 소형 로컬 모델 | 연동, 안전, 회귀, trace shape, timeout 처리 | contract/smoke/eval scorer 통과, ACL 0건, citation/refusal gate 유지 |
| `company-quality` | 회사 Qwen3.6 35B/vLLM | 최종 품질과 운영 검증 | Golden Test 통과율, 한국어 업무 톤, 답변 자연스러움, 추천 이유 설득력, latency/timeout report 통과 |

운영 원칙:

- 로컬 Qwen3 8B는 빠른 회귀 검증용이다. 배관과 안전장치가 깨졌는지 보는 모델이지 최종 품질 승인 모델이 아니다.
- 회사 Qwen3.6 35B/vLLM은 최종 품질 검증용이다. 단, ACL, citation, trace, audit gate를 먼저 통과한 결과만 품질 평가 대상으로 삼는다.
- 두 lane은 같은 synthetic corpus, 같은 principal context, 같은 retrieval/citation 정책을 사용한다.
- 회사 Qwen3.6 35B/vLLM에는 ACL 통과 chunk, 답변 초안, citation record, trace ID만 전달한다. 권한 없는 chunk, pre-ACL candidate, raw object URI, ACL 내부 상세는 전달하지 않는다.
- 모델 ID, vLLM endpoint, timeout, p50/p95 latency, failed case, human review score를 regression report에 남긴다.
- 내부 endpoint와 credential은 문서에 직접 쓰지 않고 설정값 또는 배포 secret으로 관리한다.
...
- `citation_acl_recheck`: 출력 직전 citation chunk가 여전히 principal에게 허용되는지

### Human Review

회사 Qwen3.6 35B/vLLM lane에서 사람이 보는 항목:

- 답변이 업무적으로 유용한가
- 답변이 문서 내용을 왜곡하지 않았는가
- citation이 핵심 주장과 충분히 직접적인가
- 거절 문구가 과도하거나 부족하지 않은가
- 한국어 업무 톤이 자연스럽고 과장되지 않는가
- 추천 또는 판단이 있을 때 이유가 설득력 있게 정리되는가
- 응답이 장황하지 않고 실무자가 바로 사용할 수 있는가
...
6. Local Qwen3 8B smoke lane when model runtime is available
...
6. local-regression lane 결과 확인
7. company-quality lane에서 Qwen3.6 35B/vLLM Golden Test 실행
8. 한국어 업무 톤/추천 이유 human review
9. Eval/Trace UI review
10. report artifact 저장
11. baseline diff 생성
...
Local-regression lane command shape:
...
The company-quality lane performs a single OpenAI-compatible `/v1/chat/completions` probe before the eval run. The report stores only safe provenance fields: validation lane, provider, model ID, endpoint alias, timeout, latency, served model, token usage, and a short response preview.
...
4. local-regression lane 실행
5. company-quality lane 실행
6. model/vector latency 기록
7. vLLM health, timeout, concurrency smoke 기록
8. install report 승인
...
- latency p50/p95
- timeout count
- Korean business tone score
- recommendation rationale score
- failed case list
...
7. `QUALITY_REGRESSION`: 회사 35B/vLLM Golden Test 또는 human review 기준 미달
8. `UI_FLOW_BREAK`: 핵심 flow면 중단
...
- case 기대값 오류: QA/Eval
- 회사 vLLM 품질/timeout 문제: AI Runtime + DevOps/MLOps + QA/Eval
...
- company-quality model timeout/concurrency smoke
...
- local-regression lane만 통과한 상태는 release candidate가 아니다. company-quality lane 결과와 human review가 있어야 품질 승인으로 본다.

## 15. 즉시 다음 액션

1. local-regression lane을 로컬 Qwen3 8B 실행 프로파일로 고정
2. company-quality lane의 회사 Qwen3.6 35B/vLLM endpoint alias와 timeout 기준 확정
3. Golden Test report에 validation lane/model provider/model endpoint alias/model version 필드 추가
4. 한국어 업무 톤과 추천 이유 human review rubric을 eval report에 연결
5. synthetic corpus v0.1을 company-quality lane으로 1회 dry run
