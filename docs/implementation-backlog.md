...
- Agent Studio now includes a lifecycle workflow on `/agents` for creating a draft agent and v1 version, validating, publishing, syncing the API catalog, and then running published-version Test Chat with `agent_version_id`/`knowledge_source_ids`, citation chips, guardrail/citation status, no-context refusal state, and Trace links.
